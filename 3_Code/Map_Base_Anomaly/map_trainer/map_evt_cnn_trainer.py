...
early_stopping = EarlyStopping(monitor='loss', patience=20, mode='min', verbose=1, restore_best_weights=True)
model_cnn.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

model_json = model_cnn.to_json()
with open("output/model_cnn_evt_map.json", "w") as json_file:
    json_file.write(model_json)
print(model_cnn.summary())

# --------------------- Weight Initialize -----------------------------------------------
try:
    model_cnn.load_weights('output/weight_cnn_evt_map.h5')
...
while 1:
    ...
    image_files_v2_nor = glob.glob('mapDB_v2/0/evt/*.*')
    # image_files_v2_mov_nor = glob.glob('mapDB_v2/moving_agent/evt/*.*')
    image_files_v2_fdb = glob.glob('mapDB_v2/feedback/caution/evt/*.*')
    print('Evt. CNN normal DB:', 'v2_0', np.shape(image_files_v2_nor), 'v2_caution', np.shape(image_files_v2_fdb))
    image_files_nor = image_files_v2_nor + image_files_v2_fdb
    x_normal_train = ofn.data_read_dstack(image_files_nor)

    # image_files_abn_1 = glob.glob('mapDB/evt/1/*.bmp')
    image_files_abn_2 = glob.glob('mapDB_v2/1/evt/*.bmp')
    image_files_abn = image_files_abn_2
    print('Evt. CNN abnormal DB:', 'v2', np.shape(image_files_abn_2))
    x_abnormal_train = ofn.data_read_dstack(image_files_abn)

    print('Evt TrainDB: Nor', np.shape(x_normal_train), 'Abn', np.shape(x_abnormal_train))
    ...
    # history = model_cnn.fit_generator(datagen.flow(x_train, y_train, batch_size=trn_bat_size), steps_per_epoch=int(np.shape(x_train)[0] / trn_bat_size), epochs=200, verbose=2)
    print('Normal Evt Train:', np.shape(x_normal_train)[0], 'Result: ',  ofn.cnn_prediction_result(model_cnn, x_normal_train))
    print('Abnormal Evt  Train:', np.shape(x_abnormal_train)[0], 'Result:', ofn.cnn_prediction_result(model_cnn, x_abnormal_train))
    ...
    np.savez_compressed('output/maha_weight_evt_map.npz', n_mu=n_mu, n_inv_cov=n_inv_cov, a_mu=a_mu, a_inv_cov=a_inv_cov)

    # ------------------- Trained Weight Save --------------------------------------
    model_cnn.save_weights('output/weight_cnn_evt_map.h5')
    print('Elv/Thr CNN trained weight saved')
