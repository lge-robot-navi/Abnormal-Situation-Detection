/**
 * @file		fixed_thermal_layer.cpp
 * @date		Mar 22, 2019
 * @author  	Kiin Na (kina4147@etri.re.kr)
 * @brief   	This file includes function definitions that are declared in fixed_thermal_layer class.
 * @remark
 * @warning
 *
...
osr_map::fixed_thermal_layer::fixed_thermal_layer(ros::NodeHandle _nh, int _period, int _robot_id, std::string _robot_frame_id, std::string _map_frame_id)
...
void osr_map::fixed_thermal_layer::set_comm(std::string _layer_topic, std::string _pose_topic, std::string _depth_image_topic)
{
  	pub_layer_ = nh_.advertise<grid_map_msgs::GridMap>(_layer_topic, 1, true);
	sub_pose_ = nh_.subscribe(_pose_topic, 1, &osr_map::fixed_thermal_layer::subscribe_pose_callback, this);
//	sub_pointcloud_ = nh_.subscribe(_pointcloud_topic, 1, &osr_map::fixed_thermal_layer::subscribe_pointcloud_callback, this);
	sub_depth_image_ = nh_.subscribe(_depth_image_topic, 1, &osr_map::fixed_thermal_layer::subscribe_depth_image_callback, this);
}
void osr_map::fixed_thermal_layer::set_map_param(float _length_x, float _length_y, float _resolution)
{
	length_x_ = _length_x;
	length_y_ = _length_y;
...
void osr_map::fixed_thermal_layer::set_z_param(float _offset_z, float _max_z, float _thickness)
{
	offset_z_ = _offset_z;
	max_z_ = _max_z;
	thickness_ = _thickness;
}
void osr_map::fixed_thermal_layer::set_distance_param(float _min_dist, float _max_dist)
{
	min_dist_ = _min_dist;
	max_dist_ = _max_dist;
}
void osr_map::fixed_thermal_layer::reset()
{
...
// read? set?
void osr_map::fixed_thermal_layer::set_camera_param(float _fx, float _fy, float _cx, float _cy, float _camera_x, float _camera_y, float _camera_z, float _rx, float _ry, float _rz)
{
	// Read File
...
void osr_map::fixed_thermal_layer::run()
{
	period_.reset();
	reset();
	...
	for (int y = 0; y < depth_image_.rows; y++)
     {
         ...
         for (int x = 0; x < depth_image_.cols; x++)
         {
             ...
             if (d > min_dist_ && d < max_dist_)
             {
                 ...
                 pointcloud_ptr_->push_back(pcl::PointXYZ(pw.z + camera_y_, -pw.x -camera_x_, -pw.y + camera_z_));

             }
         }
     }
//     std::cout << "#points : " << pointcloud_ptr_->size() << std::endl;
     // voxel filter //
     // Create the filtering object
     pcl::VoxelGrid<pcl::PointXYZ> vxfilter;
     vxfilter.setInputCloud (pointcloud_ptr_);
     vxfilter.setLeafSize (0.2f, 0.2f, 0.2f);
     vxfilter.filter (*pointcloud_ptr_);
//     std::cout << "#filtered points : " << pointcloud_ptr_->size() << std::endl;

  	 pcl::transformPointCloud(*(pointcloud_ptr_), *(pointcloud_ptr_),
...
void osr_map::fixed_thermal_layer::subscribe_pose_callback(const nav_msgs::Odometry::ConstPtr& _msg) {

	pose_ = *_msg;
}
void osr_map::fixed_thermal_layer::subscribe_depth_image_callback(const sensor_msgs::Image::ConstPtr& _msg)
{
