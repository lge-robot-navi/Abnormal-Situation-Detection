...
void osr_map::fixed_object_layer::set_camera_param(double _fx, double _fy, double _cx, double _cy, double _camera_x, double _camera_y, double _camera_z, double _rx, double _ry, double _rz)
{
    ...
    param.cy = _cy;
    param.w = 0.0;
    camera_.set_intrinsic_parameters(param);

    evl::SE3 se3;
	double x = _camera_x;
	double y = _camera_y;
	double z = _camera_z;
	double pan = ((_rx) * CV_PI / 180.0);
	double tilt = ((_ry) * CV_PI / 180.0);
	double roll = ((_rz) * CV_PI / 180.0);
	se3.setPosePanTiltRoll(x, y, z, pan, tilt, roll);
	camera_.set_extrinsic(se3);
}
void osr_map::fixed_object_layer::set_camera_range_param(double _image_from_width, double _image_to_width, double _image_from_height, double _image_to_height)
{
	image_from_width_ = _image_from_width;
	image_to_width_ = _image_to_width;
	image_from_height_ = _image_from_height;
	image_to_height_ = _image_to_height;
}
...
void osr_map::fixed_object_layer::run()
{
	...
	// multi-layered map
	// Update
	layer.add("object_update", 0);//robot_id_); // it should get range
    ...
    image_pts.push_back(cv::Point2d(image_from_width_, image_from_height_));
    image_pts.push_back(cv::Point2d(image_to_width_, image_from_height_));
    image_pts.push_back(cv::Point2d(image_to_width_, image_to_height_));
    image_pts.push_back(cv::Point2d(image_from_width_, image_to_height_));
    std::vector<cv::Point2d> world_pts;
	grid_map::Polygon polygon;
	polygon.setFrameId(map_frame_id_);
    camera_.unproject_ground(image_pts, world_pts, valid);
    std::cout << std::endl;
    for(int i = 0; i < world_pts.size(); ++i){
		Eigen::Vector4f lpos(world_pts[i].y, -1.0 * world_pts[i].x, 0, 1);
		Eigen::Vector4f gpos = l2g_pos * lpos;
		polygon.addVertex(grid_map::Position(gpos(0),  gpos(1)));
    }
	for (grid_map::PolygonIterator iterator(layer, polygon);
	  !iterator.isPastEnd(); ++iterator) {
		layer.at("object_update", *iterator) = robot_id_;
	}
