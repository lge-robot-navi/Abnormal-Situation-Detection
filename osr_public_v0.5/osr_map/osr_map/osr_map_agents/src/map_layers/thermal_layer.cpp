...
//	pcl::transformPointCloud(*(pointcloud_ptr_), *(thermal_points_ptr), Eigen::Vector3d(0.00858674282799, 0.103442391639, 0.113050548439)
//							, Eigen::Quaterniond(0.514381255168, -0.51859133471, 0.483379901823, -0.482513028227));
	...
	for(int idx = 0; idx < thermal_points_ptr->size(); ++idx)
	{
		...
		if(thermal_points_ptr->at(idx).z == 0){continue;}

		pcl::PointXYZ index;
		index.x = (thermal_to_lidar_(0, 0)*thermal_points_ptr->at(idx).x + thermal_to_lidar_(0, 3)) / thermal_points_ptr->at(idx).z + thermal_to_lidar_(0, 2);
		index.y = (thermal_to_lidar_(1, 1)*thermal_points_ptr->at(idx).y + thermal_to_lidar_(1, 3)) / thermal_points_ptr->at(idx).z + thermal_to_lidar_(1, 2);
	...
	for(int idx = 0; idx < lidar_points_ptr->size(); ++idx)
	{
		...
		float max_temp = static_cast<float>(thermal_image_.at<uchar>(indices_ptr->at(idx).y, indices_ptr->at(idx).x));
		for(int i = row - half_wsize; i <= row + half_wsize; ++i){
			for(int j = col - half_wsize; j <= row + half_wsize; ++j){
				if(static_cast<float>(thermal_image_.at<uchar>(i, j)) > max_temp){
					max_temp = static_cast<float>(thermal_image_.at<uchar>(i, j));
				}
			}
		}
		float temp = max_temp;
		// denormalization
		temp = min_thermal_ + temp * (max_thermal_ - min_thermal_) / 255.0;
		temp = std::max(temp, min_thermal_);
		temp = std::min(temp, max_thermal_);
		if(layer.isInside(point)){
			grid_map::Index index;
			layer.getIndex(point, index);

			if(layer.isValid(index, "thermal")){
				layer.atPosition("thermal", point) = std::max(layer.atPosition("thermal", point), temp);
			}
			else{
				layer.atPosition("thermal", point) = temp;
				layer.atPosition("thermal_update", point) = 1;
			}
		}
	}
	// map to publish
...
void osr_map::thermal_layer::subscribe_thermal_callback(const sensor_msgs::Image::ConstPtr& _msg)
{
    ...
    try
    {
    	...
    	thermal_ptr->image.convertTo(thermal_image_, CV_8U);
    }
