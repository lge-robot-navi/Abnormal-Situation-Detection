/**
 * @file		osr_map_server_multiport.cpp
 * @date		Mar 22, 2019
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "osr_map_server_multiport");
	ros::NodeHandle nh("~");
	...
	std::vector<unsigned int> ports = osr_map::get_ports(local_id, data_type, robot_ids);
	...
	std::map<std::string, std::vector<std::string>> layers_map;
	if(object_map_on){
		...
		layers_map.insert({"object_update", layers});
		all_layers.insert(all_layers.end(), layers.begin(), layers.end());
		all_layers.push_back("object_update");
	}
	if(elevation_map_on){
		std::vector<std::string> layers;
		layers.push_back("elevation");
		update_layers.push_back("elevation_update");
		layers_map.insert({"elevation_update", layers});
		all_layers.insert(all_layers.end(), layers.begin(), layers.end());
		all_layers.push_back("elevation_update");
	}
	if(thermal_map_on){
		std::vector<std::string> layers;
		layers.push_back("thermal");
		update_layers.push_back("thermal_update");
		layers_map.insert({"thermal_update", layers});
		all_layers.insert(all_layers.end(), layers.begin(), layers.end());
		all_layers.push_back("thermal_update");
	}
	boost::asio::io_service io_srv;
	std::vector<std::shared_ptr<osr_map::udp_receiver>> receivers;
	boost::thread_group tgroup;
	for (std::vector<unsigned int>::iterator it = ports.begin(); it != ports.end(); ++it){
		receivers.push_back(std::make_shared<osr_map::udp_receiver>(io_srv, *it));
		tgroup.add_thread(new boost::thread(boost::bind(&osr_map::udp_receiver::receive_loop, receivers.back())));
	}
    ...
    if(load_map_on)
    {
    	if(!manager.load_map_bag(map_path, map_name)){
    		manager.initialize_map(grid_map::Position(position_x, position_y), grid_map::Length(length_x, length_y), resolution, all_layers);
    	}
    }
    else{
    	manager.initialize_map(grid_map::Position(position_x, position_y), grid_map::Length(length_x, length_y), resolution, all_layers);
    ...
    while(ros::ok()){
        ...
        for (auto receiver_ptr : receivers){
			std::vector<osr_map::light_layered_map> tmp_layered_maps;
			if(!receiver_ptr->get_all_messages(tmp_layered_maps)){
				continue;
			}
			layered_maps.insert(layered_maps.end(), tmp_layered_maps.begin(), tmp_layered_maps.end());
        }
        if(layered_maps.empty()){
        	continue;
        }
		...
		for(std::vector<int>::iterator it = robot_ids.begin(); it != robot_ids.end(); ++it){
			if(manager.overwrite_map(robot_maps[*it], layers_map)){
					ROS_INFO_STREAM("[ " << robot_names[*it] << " ]'s [ " << robot_seqs[*it] << " ] th map is received.");
			}
		}
